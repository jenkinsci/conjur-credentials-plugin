...
public class AnnotationMapper {
    ...
    public static Map<String, String> map(StandardCredentials cred) {
        ...
        if (cred instanceof ConjurSecretUsernameCredentials) {
        ...
        } else if (cred instanceof ConjurSecretUsernameSSHKeyCredentials) {
            ...
            conjurization.put("variable:annotation:jenkins_credential_type_alt", TYPE_STRING);
        } else if (cred instanceof ConjurSecretStringCredentials) {
    ...
    public static String getCredentialType(StandardCredentials cred) {
        if (cred instanceof ConjurSecretUsernameCredentials)    return TYPE_USERNAME;
        if (cred instanceof ConjurSecretUsernameSSHKeyCredentials) return TYPE_SSH_KEY;
        if (cred instanceof ConjurSecretFileCredentials)        return TYPE_SECRET_FILE;
        if (cred instanceof ConjurSecretDockerCertCredentials)  return TYPE_DOCKER_CERT;
        if (cred instanceof ConjurSecretStringCredentials)      return TYPE_STRING;
        return TYPE_STRING;
    }
}
