		...
		if (globalConfig != null && globalConfig.getEnableJWKS()) {
			...
			if (!globalConfig.getEnableIdentityFormatFieldsFromToken())// Simplified JWT is disabled
			{
				...
				if (!identityFields.contains("jenkins_full_name")) {
					if (!(identityFields.contains("jenkins_parent_full_name")
							&& identityFields.contains("jenkins_name"))) {
						throw new RuntimeException(
								"Error validating the configuration: Customer can add additional fields to above attribute names "
